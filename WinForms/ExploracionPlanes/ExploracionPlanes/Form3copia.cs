...
{
    public partial class Form3copia : Form
    {
        ...
        public Form3copia(Plantilla _plantilla)
        {
            InitializeComponent();
        ...
        public List<PlanningItem> listaPlanes(Course curso)
        {
            ...
            {
                if (planSetup.ApprovalStatus == PlanSetupApprovalStatus.PlanningApproved || planSetup.ApprovalStatus == PlanSetupApprovalStatus.TreatmentApproved)
                {
                    lista.Add(planSetup);
                }
            }
        ...
        private void llenarDGVEstructuras()
        {
            ...
            //dgvCBCol.DataSource = Estructura.listaEstructurasID(Estructura.listaEstructuras(plan));
            asociarEstructuras();
        ...
        private void asociarEstructuras()
        {
            ...
            {
                Structure estructura = Estructura.asociarConLista(plantilla.estructuras()[i].nombresPosibles, Estructura.listaEstructuras(plan));
                if (estructura != null)
                {
        ...
        private void automatizarLista()
        {
            CHB_VolumenEstructuras.Checked = false;
            List<string> lista = Mineria.leerArchivos();
            foreach (string elemento in lista)
            {
                Mineria.Caso caso = Mineria.leerCaso(elemento);
                abrirPaciente(caso.pacienteID);
                if (listaPlanes(paciente.Courses.First()).Exists(p => p.Id == caso.planID))
                {
                    plan = paciente.Courses.First().PlanSetups.Where(p => p.Id == caso.planID).FirstOrDefault();
                    if (plan is PlanSetup && (((PlanSetup)plan).ApprovalStatus == PlanSetupApprovalStatus.PlanningApproved || ((PlanSetup)plan).ApprovalStatus == PlanSetupApprovalStatus.TreatmentApproved)) 
                    {
                        llenarDGVEstructuras();
                        llenarDGVPrescripciones();
                        aplicarPrescripciones();
